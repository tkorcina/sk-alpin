...
- [x] Výchozí logo — inline SVG (`templates/include/logo.latte`): nápis „SK-ALPIN“ (Urbanist 900) vyplněný zimní krajinou v realistickém stylu (gradienty, ne ploché barvy) — obloha s měkkým světlem slunce, vzdálený hřeben v oparu, skalnatý hřeben se světlou a stinnou stranou a sněhovou čárou, zasněžený svah s lyžařskými stopami. Bez zeleného lesa. `viewBox` je oříznutý přesně na text (změřeno přes `getBBox`), aby logo lícovalo s levým okrajem obsahu; výška 64 px. V hlavičce je jen logo (bez textového názvu). Použije se, dokud není v Nastavení nahrané vlastní logo.
- [x] Význam polí v Nastavení upřesněn: **Motto** = text v hero bloku nad tlačítkem „Podívejte se na naše akce“, **Popisek webu** = úvodní odstavec na homepage pod hero blokem (`.lead-text`) + meta description; obsah stránky `home` ze sekce Stránky se vykreslí pod ním jen pokud není prázdný (seed vyprázdněn, aby text nebyl dvakrát), **Název webu** = title okna.

- [x] **Font Raleway** (dodal uživatel do `www/src/fonts/raleway`) — použita variabilní verze (`Raleway-Variable.ttf/.woff2` + italic, woff2 vytvořeno přes fonttools), `_font-raleway.scss`, `body` s `font-feature-settings: 'lnum'` (řádkové číslice u dat), preload v layoutu, logo přepnuto na Raleway 900 a `viewBox` přeměřen (`37.6 9 245 43`).
- [x] **Fáze 5 — SEO:** `<title>` + meta description + canonical + Open Graph/Twitter meta v layoutu (výchozí z Nastavení, detailové stránky přes `setMeta()` v `PublicModule\BasePresenter` — popis z obsahu zkrácený na 160 znaků, OG obrázek z aktuality/galerie/hero). `sitemap.xml` generuje všechny statické stránky + publikované akce, aktuality a galerie. `robots.txt` zakazuje `/administrace/` a odkazuje na sitemap (produkční URL `https://sk-alpin.cz`).
- [x] **Fáze 5 — Favicon:** generováno PHP/GD skriptem (zelený zaoblený čtverec, dva zasněžené štíty, slunce) do `www/images/favicons` (16/32/180/192/512 PNG, `favicon.svg`, `site.webmanifest`) + `www/favicon.ico` (ICO s PNG položkami). Chybová stránka 4xx má správný title, favicon a lokální CSS místo Google Fonts.
- [x] **Fáze 5 — GDPR/cookie lišta:** `include/cookieBar.latte` + JS v `base.js` (volba v `localStorage` `skCookieConsent`). V Nastavení nové pole *Google Analytics — Measurement ID* (klíč `analyticsId`, migrace `Version20260922160000`). Bez ID lišta jen informuje o nezbytných cookies (tlačítko Rozumím); s ID nabízí „Pouze nezbytné“ / „Souhlasím“ a gtag.js se načte až po souhlasu (`anonymize_ip`).

## Rozpracováno / další kroky

- [ ] **Ověřit po obnovení prostředí:** 22. 9. odpoledne se na hostiteli zastavil nginx i MySQL (běží Docker stack `nas-nutriadapt` s `local_proxy` na portu 80). Zbývá: spustit migraci `Version20260922160000` (`php <scratchpad>/cli.php migrations:migrate`), reset `temp/cache`, projít web (SEO meta, sitemap.xml, favicon, cookie lišta, Raleway) a formulář Nastavení s polem Analytics. Kód je zkontrolován `php -l` a `latte-lint`, CSS/JS buildy proběhly.
...
- [ ] Fáze 5 — zbývá: OG obrázek jako statický PNG (teď jen z nahraných fotek), testování na mobilu, nasazení na produkci + záloha DB.
...
- **2026-09-22** — Font Raleway, Fáze 5 (SEO meta/OG/sitemap/robots, favicony, cookie lišta + GA v Nastavení). Netestováno přes HTTP — prostředí nedostupné (viz další kroky). Pro testy bez nginx: `php -S 127.0.0.1:8099 -t www <scratchpad>/debug-index.php` (router servíruje statické soubory a spouští Nette v debug módu). Úklid pro sudo navíc: `sudo rm -rf temp/cache.old8 temp/proxies.old-www www/dist/fonts.old-root www/dist/js.old-root`.
- **2026-09-22** — Popisek webu z Nastavení se zobrazuje na homepage (uživatel ho změnil a čekal, že se propíše). Úklid pro sudo: `sudo rm -rf temp/cache.old7`.
- **2026-09-22** — Výplň loga předělána do realističtějšího stylu (gradienty, stínované skály, bez lesa). Úklid pro sudo: `sudo rm -rf temp/cache.old6`.
- **2026-09-22** — Logo předěláno na nápis „SK-ALPIN“ v lyžařském stylu, hlavička bez textu, hero text = Motto. Úklid pro sudo navíc: `sudo rm -rf temp/cache.old3 temp/cache.old4 temp/cache.old5`.
