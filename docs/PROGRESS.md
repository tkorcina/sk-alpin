...
- [x] Výchozí logo — inline SVG (`templates/include/logo.latte`): nápis „SK-ALPIN“ (Urbanist 900) vyplněný zimní krajinou — nebe se sluncem, zasněžené štíty, bílý svah s lyžařskými stopami, les dole. `viewBox` je oříznutý přesně na text (změřeno přes `getBBox`), aby logo lícovalo s levým okrajem obsahu; výška 64 px. V hlavičce je jen logo (bez textového názvu). Použije se, dokud není v Nastavení nahrané vlastní logo.
